...
| INV-14 | 输入接缝注册/注销成对：快捷键（keymap）、滚轮/指针监听、拖拽期 body 样式副作用必须与挂载源同源清理——消费方清理函数与注册同函数对，卸载/重挂不得残留监听或全局样式 | SR2-KEY-01/02、SR2-UIK-01 规约（2026-08-23 P7-A 开单引入，B4 防线后首批 SR2 工单） | 单测（keymap 成对注销/覆盖幂等用例，SR2-KEY-01 实现时落地）+ 人审（消费方清理同源） | 未锚定（2026-08-23 登记；单测锚随 SR2-KEY-01 实现单元补，届时回写本行） |
