...
| INV-14 | 输入接缝注册/注销成对：快捷键（keymap）、滚轮/指针监听、拖拽期 body 样式副作用必须与挂载源同源清理——消费方清理函数与注册同函数对，卸载/重挂不得残留监听或全局样式 | SR2-KEY-01/02、SR2-UIK-01 规约（2026-08-23 P7-A 开单引入，B4 防线后首批 SR2 工单） | 单测（keymap.test 九用例：成对注销行为面+add/remove 同函数配对面，2026-08-23 落地）+ 人审（消费方清理同源） | 已锚定（模块级锚=SR2-KEY-01；消费方级（wheel/指针/body 样式）锚随 SR2-KEY-02/UIK-01 实现补） |
