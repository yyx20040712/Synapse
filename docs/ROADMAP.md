...
> **执行实况注记（2026-08-27 用户实锤）**：用户视检亲报「连续翻页未实现」
> ——需求二次确认（首次点名 2026-08-23；发现档=
> docs/reports/2026-08-27_visual-check-findings.md 发现 1）。P8 立项工单化，
> 排期=标签标题缺陷修复与 SR2-ENR 工单组之后；F-aware 接口冻结面不变。
...
> **P7-H 收官 ✅（2026-08-27 06ea570/8491489/c8758ea/e7b5836/d64aec4——
> 三屋五连单，全役报告=docs/reports/2026-08-27_lineage-campaign.md）**：
> **SR2-LG-01 ✅**（06ea570——迁移 004 ADR-0014 DDL 字面+repo+草稿导入
> 全有或全无替换式+lineage 域立域十一域穷举+INV-27 树单父登记（守卫宿主
> =service 写面））。**SR2-LG-02 ✅**（8491489——RT tidy tree 零依赖手写
> （**回炉 W1：轮廓帧索引改年份层序**——树深度索引致叔侄同年 x 重叠被门一
> esbuild 机器证伪）+只读 SVG 画布 pan/zoom INV-14+脉络第四视图 E4+store
> 三态；回炉 W2=空态早退删除）。**SR2-LG-03 ✅**（c8758ea——写四通道+
> LineageDomainError CONFLICT 升级（主控追认——toAppError 折叠机制致
> reason 透传不成立）+交互编辑+自动保存 INV-04 同型+**导入入口回炉补**
> （LG-01 条款消费窗口）+退出聚合（INV-22 扩面））。**SR2-LG-04 ✅**
> （e7b5836——侧板四区+ai-note-style 跨域白名单+双击跳转链（锚递达自裁=
> 总线载荷扩+open-paper-anchor 模块+INV-20 消费方级）；**收口 staging
> 失误漏提交新文件——LG-05 实现者申报拦截，cbc6b1c 补提交**）。
> **SR2-LG-05 ✅**（d64aec4——e2e 全链 4 用例（八验收面）+守卫改仅依赖组
> （主控裁定）+**P7-C e2e 崩溃跨单修复：App 组合根 `||` 短路违反 Rules
> of Hooks（主控 LG-03 亲笔引入——调试代理根因定位+回归锁）**）。终态
> 基线：verify exit 0（**86 文件 615 用例**）+e2e **20/20**+locks **131**
> +工单 104 open **0**。INV-20/22/27 随单锚定。P7 主线（A~H）完成。
...
> **P8 工单化起步（2026-08-27）**：SR2-ENR-01/02（含金量供给组，D3-A 档）
> 票面双门 PASS 落地（票面终稿+审计=scripts/audits/enr-ticketing-*；骨架
> +registry 登记，实施待三屋单元领取，ENR-01→02 串行）。P8 战役编排与
> D1~D5 实施层预裁=docs/reports/2026-08-27-p8-proposal.md；M0 视检发现
> 台账（三缺陷处置+五项用户自填区）=docs/reports/2026-08-27_visual-check-
> findings.md。
