...
主线 **P7-A → P7-B → P7-F → P7-C → P7-G**（依赖链：标注菜单是「添加笔记」入口；tab 是灰点载体；
连续滚动先定几何模型——笔记跳转/锚定一次到位；C 消费 A/B/F 的接缝；G（AI 传感器链条，2026-08-25
增量裁决）消费 C 的语料面与装配规约——P7-F 与 G 零依赖可穿插）；**P7-D** 并行
...
| 验收 | 工单序列 SR2-AI-01~05（技术路线=docs/reports/2026-08-25_ai-module-plan.md）：repo 单测/提取管线多页夹具断言/导出 golden+结构断言（ADR-0011 口径）/e2e 全链/queue 幂等测试；蓝图 B0 §4.1 裁决记录+§4.2 七问 schema 为规约来源 |
