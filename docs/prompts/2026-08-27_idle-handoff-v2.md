...
本会话按卡 §7 完成工单化单元，未触发 §5 终止。**待裁决（2026-08-27 用户
已全裁——逐条结论与出处）**：
1. 预裁决表升格 ADR → **是**（ADR-0016，含节奏修订）。
2. companion 双目录 CLI → **接受**（ADR-0015 追认行）。
3. 两常量值 → **认可**（ADR-0015 追认行）。
4. 批一视检三件 → **①②仍挂账，③并 08 联动验**（六轮裁决后口径=ADR-0017
   同日批二首单已落地；最新任务书=next-session-handoff-v8）。
